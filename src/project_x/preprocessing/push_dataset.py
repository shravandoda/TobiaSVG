...
from project_x.constants import (
    DATA_PROCESSING_SEED,
    DATASETS,
    HF_TOKEN,
    SPLITS,
    DatasetSpec,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Path to a prepared dataset or DatasetDict. "
            "Omit to merge data/processed/datasets into TobiaSVG."
        ),
    )
    parser.add_argument(
        "--private",
...
def get_dataset_dict(dataset_path: Path, spec: DatasetSpec) -> DatasetDict:
    ...
    if val is None and test is None:
        ...
        train_test_split = train.train_test_split(
        ...
        train_val_split = train.train_test_split(
            test_size=SPLITS["val"] / (1 - SPLITS["test"]),
            seed=DATA_PROCESSING_SEED,
        )
...
def main() -> None:
    args = parse_args()
    dataset = (
        load_from_disk(str(args.dataset_path))
        if args.dataset_path is not None
        else merge_datasets()
    )
    dataset.push_to_hub(repo_id=args.repo_id, private=args.private, token=HF_TOKEN)
