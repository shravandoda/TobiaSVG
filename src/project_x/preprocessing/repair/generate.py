...
from project_x.constants import DATASETS, DatasetSpec
from project_x.preprocessing.paths import OUTPUT_ROOT, build_split_output_path
from project_x.preprocessing.svg import SVG_NS, render_svg_to_png
...
def corrupt_style(svg: str, rng: random.Random, level: str) -> str:
    ...
    for element in rng.sample(candidates, k=min(update_count, len(candidates))):
        ...
        if attribute == "opacity":
            ...
            color = choose_different_color(element.attrib[attribute], rng)
...
def corrupt_text(svg: str, rng: random.Random, level: str) -> str:
    ...
    candidates = [
        element
        for element in root.iter()
        if local_tag_name(element) == "text"
        and element.text
...
def order_corruptions_for_application(corruption_types: list[str]) -> list[str]:
    return sorted(
        corruption_types,
        key=lambda corruption_type: corruption_type == "truncation",
    )
...
def render_difference_metrics(
    ...
    for clean_pixel, corrupted_pixel in zip(
        clean_pixels,
        corrupted_pixels,
        strict=True,
    ):
        pixel_changed = False
        for clean_channel, corrupted_channel in zip(
            clean_pixel,
            corrupted_pixel,
            strict=True,
        ):
...
def image_pixels(image: Image.Image) -> list[tuple[int, int, int, int]]:
    ...
    return cast(list[tuple[int, int, int, int]], pixels)
...
def validate_render_difference(
    ...
    *,
    ...
    if render_mse < min_render_mse:
        raise CorruptionError(f"render MSE {render_mse:.6f} < {min_render_mse:.6f}")
